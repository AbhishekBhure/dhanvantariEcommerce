...
type Dashboard = { ordersToday: number; ordersPending: number; revenueToday: number; totalRevenue: number; totalProducts: number; lowStockProducts: { id: string; name: string; stock: number }[]; recentOrders: { id: string; orderNumber: string; status: string; total: number; createdAt: string; user: { name: string } | null }[] };
type Response = { data: Dashboard };
...
export default function AdminHomePage() {
  ...
  useEffect(() => { void api<Response>("/admin/dashboard").then((response) => setData(response.data)).catch((requestError) => { if (requestError instanceof ApiError && requestError.status === 401) router.replace("/login"); else setError(requestError instanceof ApiError && requestError.status === 403 ? "Admin access required." : "Could not load dashboard."); }); }, [router]);
  async function signOut() { await api("/auth/logout", { method: "POST" }); router.replace("/login"); }
  if (error) return <main className="admin-container"><p className="text-red-700">{error}</p></main>;
  if (!data) return <main className="admin-container text-slate-500">Loading dashboard...</main>;
