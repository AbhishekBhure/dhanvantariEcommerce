...
export default function AdminLoginPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setLoading(true); setError(""); const result = await dispatch(loginAdmin({ email, password })); if (loginAdmin.fulfilled.match(result)) router.replace("/"); else setError(result.payload as string ?? "Unable to sign in."); setLoading(false); }
