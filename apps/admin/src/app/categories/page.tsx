...
type Category = { id: string; name: string; slug: string; description: string | null; isPublished: boolean; sortOrder: number; _count?: { products: number } };
type Response = { data: { categories: Category[] } };
...
export default function CategoriesPage() {
  ...
  async function loadCategories() {
    try { const response = await api<Response>("/admin/categories"); setCategories(response.data.categories); }
    catch (requestError) { setError(requestError instanceof ApiError ? requestError.message : "Could not load categories."); }
  }
  useEffect(() => { void loadCategories(); }, []);

  async function createCategory(event: FormEvent) {
    event.preventDefault(); setError(""); setMessage("");
    try { await api("/admin/categories", { method: "POST", body: JSON.stringify({ name, description: description || null, isPublished: true, sortOrder: categories.length }) }); setName(""); setDescription(""); setMessage("Category created successfully."); await loadCategories(); }
    catch (requestError) { setError(requestError instanceof ApiError ? requestError.message : "Could not create category."); }
  }

  async function toggleCategory(category: Category) {
    setSavingId(category.id); setError("");
    try { await api(`/admin/categories/${category.id}`, { method: "PATCH", body: JSON.stringify({ isPublished: !category.isPublished }) }); await loadCategories(); }
    catch (requestError) { setError(requestError instanceof ApiError ? requestError.message : "Could not update category."); }
    finally { setSavingId(""); }
  }
